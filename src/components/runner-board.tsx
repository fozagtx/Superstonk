      ...
      <div className="mb-3 flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Top PreStocks moves by daily, weekly, and monthly window.
          </p>
        </div>
      </div>
            ...
            <div className="overflow-hidden rounded-xl bg-card ring-1 ring-foreground/10">
              {runners[window].length === 0 ? (
                <p className="p-5 text-sm text-muted-foreground">
                  History warming up — runners appear as DEX candles arrive.
                </p>
              ) : (
                <div className="divide-y divide-border">
                  {runners[window].slice(0, 8).map((runner, index) => (
                    <Link
                      key={runner.symbol}
                      href={`/token/${runner.symbol}`}
                      className="grid grid-cols-[2rem_1fr_auto_auto] items-center gap-3 px-4 py-3 transition-colors hover:bg-muted/50 sm:grid-cols-[2rem_2fr_1fr_1fr_1fr_auto]"
                    >
                      ...
                      <span
                        className={`num text-right font-medium ${runner.changePct >= 0 ? "text-discount" : "text-overpriced"}`}
                      >
                        {fmtPct(runner.changePct)}
                      </span>
                      <span className="hidden text-right text-xs text-muted-foreground sm:block">
                        {runner.volume24hUsd == null
                          ? "—"
                          : fmtUsd(runner.volume24hUsd)}
                      </span>
                      <span className="hidden justify-self-end sm:block">
                        <VerdictChip verdict={runner.verdict} />
                      </span>
                      <span className="num rounded-md bg-muted px-2 py-1 text-xs">
                        {runner.score}
                      </span>
                    </Link>
                  ))}
