...
export function volatilityPct(candles: Candle[], days: number): number | null {
  ...
  const closes = candles
    .slice(-Math.min(candles.length, days + 1))
    .map((c) => c.c);
...
export function rangePosition(candles: Candle[], days: number): number | null {
  ...
  return Math.min(
    1,
    Math.max(0, (window[window.length - 1]!.c - minLow) / (maxHigh - minLow)),
  );
}

export interface Signal {
  kind:
    "momentum" | "premium" | "liquidity" | "divergence" | "volatility" | "flow";
...
export function buildSignals(i: AnalysisInput): Signal[] {
  ...
  if (i.change7d != null && i.change7d > 10) {
    signals.push({
      kind: "momentum",
      tone: "bullish",
      text: `Up ${magnitude(i.change7d)} over 7d — strong weekly runner`,
    });
  } else if (i.change7d != null && i.change7d < -10) {
    signals.push({
      kind: "momentum",
      tone: "bearish",
      text: `Down ${magnitude(i.change7d)} over 7d — weekly momentum is weak`,
    });
  } else if (i.change1d != null && i.change1d > 5) {
    signals.push({
      kind: "momentum",
      tone: "bullish",
      text: `Up ${magnitude(i.change1d)} today — daily runner`,
    });
  } else if (i.change1d != null && i.change1d < -5) {
    signals.push({
      kind: "momentum",
      tone: "bearish",
      text: `Down ${magnitude(i.change1d)} today — daily momentum is weak`,
    });
  }
  if (i.verdict === "discount") {
    signals.push({
      kind: "premium",
      tone: "bullish",
      text: `Trades ${magnitude(i.premiumPct)} below mark price — discount to private-market value`,
    });
  } else if (i.verdict === "overpriced") {
    signals.push({
      kind: "premium",
      tone: "caution",
      text: `${pct(i.premiumPct)} premium over mark price`,
    });
  } else {
    signals.push({
      kind: "premium",
      tone: "neutral",
      text: "Trades near mark price — fair private-market value",
    });
  }
  if (i.liquidityUsd != null && i.liquidityUsd < 50_000) {
    signals.push({
      kind: "liquidity",
      tone: "caution",
      text: `Thin liquidity (${usd(i.liquidityUsd)}) — size positions small`,
    });
  ...
  if (
    i.dexDivergencePct != null &&
    Math.abs(i.dexDivergencePct) > 10 &&
    i.dexPriceUsd != null
  ) {
    signals.push({
      kind: "divergence",
      tone: "caution",
      text: `DEX price ${usd(i.dexPriceUsd)} diverges ${magnitude(i.dexDivergencePct)} from PreStocks quote`,
    });
  }
  if (i.volatility7d != null && i.volatility7d > 8) {
    signals.push({
      kind: "volatility",
      tone: "caution",
      text: `7d volatility is ${i.volatility7d.toFixed(1)}% — expect larger moves`,
    });
  }
  if (i.buys24h != null && i.sells24h != null && i.buys24h + i.sells24h >= 5) {
    if (i.buys24h > i.sells24h * 1.5) {
      signals.push({
        kind: "flow",
        tone: "bullish",
        text: `Net buying: ${i.buys24h} buys vs ${i.sells24h} sells (24h)`,
      });
    } else if (i.sells24h > i.buys24h * 1.5) {
      signals.push({
        kind: "flow",
        tone: "bearish",
        text: `Net selling: ${i.buys24h} buys vs ${i.sells24h} sells (24h)`,
      });
    }
  }
  return signals;
...
export function score(i: AnalysisInput): number {
  ...
  if (i.dexDivergencePct != null && Math.abs(i.dexDivergencePct) > 10)
    value -= 10;
...
export function summarize(i: AnalysisInput, signals: Signal[]): string {
  ...
  const risk =
    i.liquidityUsd != null && i.liquidityUsd < 50_000
