...
export default async function TokenPage({
  ...
  const stats = [
    ...
    [
      "DEX price",
      token.dex?.priceUsd == null ? "—" : fmtUsd(token.dex.priceUsd),
    ],
    [
      "DEX divergence",
      token.dexDivergencePct == null ? "—" : fmtPct(token.dexDivergencePct),
    ],
    [
      "DEX liquidity",
      token.dex?.liquidityUsd == null ? "—" : fmtUsd(token.dex.liquidityUsd),
    ],
    [
      "DEX 24h volume",
      token.dex?.volume24hUsd == null ? "—" : fmtUsd(token.dex.volume24hUsd),
    ],
    [
      "24h buys / sells",
      token.dex?.topPool
        ? `${token.dex.topPool.buys24h} / ${token.dex.topPool.sells24h}`
        : "—",
    ],
  ];
  return (
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 pb-12">
        ...
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="num rounded-lg bg-accent/15 px-3 py-2 text-lg font-bold text-accent">
              {token.score}
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_1fr]">
          <div className="space-y-6">
            <Card>
              ...
              <CardContent>
                <PriceChart candles={token.dex?.candles ?? []} />
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <ul className="space-y-3">
                  {token.signals.map((signal, index) => (
                    ...
                    >
                      <span
                        className={`mt-1.5 size-2 shrink-0 rounded-full ${signal.tone === "bullish" ? "bg-discount" : signal.tone === "bearish" ? "bg-overpriced" : signal.tone === "caution" ? "bg-warning" : "bg-fair"}`}
                      />
            ...
            <Card>
              ...
              <CardContent>
                ...
                {token.externalUrl && (
                  ...
                  >
                    PreStocks source <ExternalLink className="size-3" />
                  </a>
                )}
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Suggestions for an agent planning a Meteora DBC launch —
                  review and simulate before signing.
                </p>
              </CardHeader>
              <CardContent className="space-y-3">
                <LaunchKitPreview kit={kit} />
                <code className="block overflow-x-auto rounded-lg bg-muted p-3 text-xs">
                  curl {appUrl}/api/v1/tokens/{token.symbol}/launch-kit
                </code>
              </CardContent>
            </Card>
          </div>
